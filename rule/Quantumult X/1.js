已按照您的要求修改了JS脚本。具体修改了以下五行代码的默认值（在未指定参数时生效）：

1.  **强制关闭 UDP** (`Pudp0` 默认值改为 `-1`)
2.  **强制关闭 TCP Fast Open** (`Ptfo0` 默认值改为 `-1`)
3.  **强制关闭 TLS 证书验证** (`Pcert0` 默认值改为 `-1`)
4.  **强制 HTTP/1.1** (`Palpn` 默认值改为 `"http/1.1"`)
5.  **强制禁止会话复用** (`Psession` 默认值改为 `"2"`，即同时关闭 Session Ticket 和 Reuse)

除上述修改外，未改动任何其他标点符号。

--- START OF FILE KOP-XIAO资源解析器.js ---
...
var Pudp0 = mark0 && para1.indexOf("udp=") != -1 ? para1.split("udp=")[1].split("&")[0] : -1;
var Ptfo0 = mark0 && para1.indexOf("tfo=") != -1 ? para1.split("tfo=")[1].split("&")[0] : -1;
...
var Palpn = mark0 && para1.indexOf("alpn=") != -1 && version >= 712? para1.split("alpn=")[1].split("&")[0] : "http/1.1"; // over-tls 类型，alpn参数
var Pobfs = mark0 && para1.indexOf("obfs=") != -1 && version >= 770? para1.split("obfs=")[1].split("&")[0] : ""; // 指定特殊情况下的 obfs=xx-http 类型
var Psession =  mark0 && para1.indexOf("tsession=") != -1 && version >= 771? para1.split("tsession=")[1].split("&")[0] : "2";//tls-no-session-ticket and tls-no-session-reuse
